...
class _IncluirTarefaPageState extends State<IncluirTarefaPage> {
  ...
  List<Usuario> _users = []; // Lista de usuários
  final FirestoreService _firestoreService = FirestoreService();

  @override
  void initState() {
    super.initState();
    _loadUsers(); // Carregar usuários ao iniciar
  }
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: _users.map<DropdownMenuItem<String>>((Usuario user) {
                  return DropdownMenuItem<String>(
                    value: user.id,
                    child: Text(user.nome),
                  );
                }).toList(),
                decoration: InputDecoration(labelText: 'Atribuir a'),
              ),
              SizedBox(height: 16),
              DropdownButtonFormField<String>(
    ...
    if (user != null) {
      try {
        await _firestoreService.addTask({
          ...
          'isCompleted': false,
          'userId': user.uid,
          'assignedTo': _selectedUser, // Adicionar usuário atribuído
